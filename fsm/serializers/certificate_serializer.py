...
from fsm.models import Font, CertificateTemplate
...
def create_certificate(certificate_template, full_name):
    ...
    if extension in ['png', 'jpg', 'jpeg', 'gif']:
        ...
        draw.text((certificate_template.name_X, certificate_template.name_Y), bidi_text, (0, 0, 0), font=font)
        draw = ImageDraw.Draw(image)

        image_io = BytesIO()
        image.save(image_io, format='png')
        certificate_file_name = f'{certificate_template.template_file.name}-{full_name}.{extension}'
        image_file = InMemoryUploadedFile(image_io, None, certificate_file_name, f'image/{extension}', image.tell, None)
